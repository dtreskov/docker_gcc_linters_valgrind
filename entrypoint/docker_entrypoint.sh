...
read -r answer
if [[ "$answer" = [Yy]* ]]; then
  valgrind --trace-children=yes --track-fds=yes --track-origins=yes --leak-check=full --show-leak-kinds=all build/test_decimal 2>&1 | tee "logs/valgrind_$(date +"%d_%m_%y__%H_%M_%S").log"
fi
...
clang-format -n *.c *.h tests/*.c tests/*.h 2>&1 | tee "logs/clang_$(date +"%d_%m_%y__%H_%M_%S").log"
...
cpplint *.c *.h tests/*.c tests/*.h 2>&1 | tee "logs/cpplint_$(date +"%d_%m_%y__%H_%M_%S").log"


# CPPCHECK static code analyzer. "-q --enable=all --inconclusive" for maximum sensitivity
cppcheck -q --enable=all --inconclusive *.c *.h tests/*.c tests/*.h 2>&1 | tee "logs/cppcheck_$(date +"%d_%m_%y__%H_%M_%S").log"
