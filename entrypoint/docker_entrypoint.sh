...
read -r answer
if [[ "$answer" = [Yy]* ]]; then
  valgrind --trace-children=yes --track-fds=yes --track-origins=yes --leak-check=full --show-leak-kinds=all build/test 2>&1 | tee "logs/test_valgrind_$(date +"%d_%m_%y__%H_%M_%S").log"
fi
